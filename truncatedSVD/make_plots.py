  ...
  for batch_split, phi_list in zip(batch_splits, phis):
    ...
    if (batch_split == 1):
      ...
      for method in evolution_methods:
        ...
        relative_errors = np.load(f"../cache/{method}/{dataset}_batch_split_{str(batch_split)}/relative_errors_phi_{str(phi)}{r_str}.npy")
        residual_norms = np.load(f"../cache/{method}/{dataset}_batch_split_{str(batch_split)}/residual_norms_phi_{str(phi)}{r_str}.npy")
