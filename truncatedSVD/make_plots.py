	...
	for batch_split, phi_list in zip(batch_splits, phis):
		...
		if batch_split == 1:
			...
			for method in evolution_methods:
				...
				plot_relative_errs(relative_errors_list, evolution_names, f"relative_errors_{dataset}_batch_split_{str(batch_split)}", title=dataset)
				plot_residual_norms(residual_norms_list, evolution_names, f"residual_norms_{dataset}_batch_split_{str(batch_split)}", title=dataset)
		...
		# Multi-batch update plots
			...
			for ii, method in enumerate(evolution_methods):
				...
				plot_stacked_relative_errs(relative_errors_list, phi_list, f"relative_errors_{dataset}_batch_split_{str(batch_split)}_{method}", dataset+", "+evolution_names[ii])
				plot_stacked_residual_norms(residual_norms_list, phi_list, f"residual_norms_{dataset}_batch_split_{str(batch_split)}_{method}", dataset+", "+evolution_names[ii])
