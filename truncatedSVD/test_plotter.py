...
def plot_stacked_residual_norms(
    errs_list,
    phi_list,
    save_dir,
    title="Residual Norms for Singular Vectors",
    ylim=None,
):
    ...
    plt.savefig(
        normpath(join(save_dir, f"resnorms.png")),
        bbox_inches="tight",
        pad_inches=0.2,
        dpi=200,
    )
...
def plot_stacked_relative_errors(
    errs_list,
    phi_list,
    save_dir,
    title="Relative Error for Singular Vectors",
    ylim=None,
):
    position = np.arange(1, errs_list[0].shape[0] + 1)
    ...
    ax.legend(loc="lower right")
    plt.savefig(
        normpath(join(save_dir, f"relerrs.png")),
        bbox_inches="tight",
        pad_inches=0.2,
        dpi=200,
    )
...
def plot_runtimes(runtimes, xval, filename, title=None):
    ...
    ax.set_yscale("log")
    ax.legend(loc="lower right")
    plt.savefig(f"../{filename}.png", bbox_inches="tight", pad_inches=0.2, dpi=200)
    plt.close()
